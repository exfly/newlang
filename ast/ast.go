...
func NewReturnStatement(tok token.Token, val Expression) Statement {
...
func NewBlockStatement(tok token.Token, stmt []Statement) Statement {
...
func NewRowBlockStatement(tok token.Token, stmt []Statement) *BlockStatement {
	return &BlockStatement{
		Token:      tok,
		Statements: stmt,
	}
...
func (bs *BlockStatement) AppendStatement(stmt Statement) {
	bs.Statements = append(bs.Statements, stmt)
}

func NewIdentifier(tok token.Token, value string) Expression {
	return &Identifier{
		Token: tok,
		Value: value,
	}
...
func NewIntegerLiteral(tok token.Token) Expression {
	lit := &IntegerLiteral{Token: tok}

	value, err := strconv.ParseInt(tok.Literal, 0, 64)
	if err != nil {
		log.Printf("could not parse %q as integer", tok)
		return nil
	}
...
func NewPrefixExpression(tok token.Token, right Expression) Expression {
...
func NewInfixExpression(tok token.Token, left, right Node) Expression {
...
func NewIfExpression(tok token.Token, cond Expression, cons, alter *BlockStatement) Expression {
	return &IfExpression{
		Token:       tok,
		Condition:   cond,
		Consequence: cons,
...
func NewFunctionLiteral(tok token.Token, name string, params []*Identifier, body *BlockStatement) Expression {
...
func NewCallExpression(tok token.Token, fn Expression, args []Expression) Expression {
...
func NewBindExpression(tok token.Token, left, value Expression) Expression {
...
func NewAssignmentExpression(tok token.Token, left, value Expression) Expression {
